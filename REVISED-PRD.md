...
| **Name** | DClaw Wiki |
| **Category** | Knowledge |
| **Tagline** | Internal Wikipedia |
| **Color** | #6366F1 |
| **Phase** | Future |
| **Port (Frontend Dev)** | 3037 (TBD — assign before build) |
| **Port (Backend Dev)** | 18107 (TBD — assign before build) |
...
|---|---------|-------------|--------------|---------------------|
| P0.1 | **AI Wiki Copilot** | Draft, organize, and update wiki pages with AI assistance. | LLM content generation + RAG over existing docs | Generate page in <2min; suggest updates from changes |
| P0.2 | **Hierarchical Pages** | Nested page structure with breadcrumbs and navigation. | AI page-structure suggestion + navigation optimization | Unlimited nesting; auto-TOC; breadcrumb navigation |
| P0.3 | **Semantic Search** | Find information across all pages using natural language. | Embedding-based retrieval + hybrid keyword | Search 10K pages in <1s; natural language queries |
| P0.4 | **Version History** | Track every change with diff, author, and rollback. | AI change-summary generation + impact analysis | Unlimited history; visual diff; one-click rollback |
...
| # | Feature | Description | AI Component | Acceptance Criteria |
|---|---------|-------------|--------------|---------------------|
| P1.1 | **Access Control** | Page-level and space-level permissions with RBAC. | AI permission-recommendation + access-anomaly detection | Role-based; public/private; expiration dates |
| P1.2 | **Templates** | Standardized templates for runbooks, SOPs, and meeting notes. | AI template suggestion + auto-population | 50+ templates; auto-fill from context; custom creation |
| P1.3 | **Comment & Discuss** | Inline comments and page-level discussions. | AI sentiment analysis + consensus detection | Threaded comments; @mentions; resolve tracking |
| P1.4 | **Import & Export** | Import from Confluence, Notion, and Markdown. | AI structure-preservation + link-fixing | Preserve 95%+ structure; fix broken links; batch import |
...
| # | Feature | Description | AI Component | Acceptance Criteria |
|---|---------|-------------|--------------|---------------------|
| P2.1 | **AI Content Maintenance** | Identify outdated pages and suggest updates. | AI staleness-detection + update-recommendation | Flag pages >90 days old; suggest updates; auto-archive |
| P2.2 | **Knowledge Graph** | Visual graph of related concepts and pages. | AI relationship-extraction + graph layout | Interactive graph; 5 relationship types; explore mode |
| P2.3 | **Q&A Bot** | Answer questions using wiki content via chat interface. | RAG over wiki + conversational LLM | Answer 80% of questions; cite sources; suggest related pages |
| P2.4 | **Analytics** | Track page views, searches, and popular content. | AI content-gap identification + recommendation | Track 15 metrics; identify gaps; suggest new pages |
...
Inspired by Confluence, Notion, Slab, Guru. Wikis are the single source of truth for teams.
